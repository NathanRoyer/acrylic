...
impl WaylandApp {
	pub fn new(acrylic_app: Application) -> (Self, EventQueue) {
		...
		event_queue.dispatch(&mut (), |_,_,_| unreachable!()).expect("Event dispatching Error!");

		let compositor = gm.instantiate_range::<WlCompositor>(0, WlCompositor::VERSION).unwrap();
		let xdg_wm_base = gm.instantiate_range::<XdgWmBase>(0, XdgWmBase::VERSION).unwrap();
	...
	pub fn frame(&mut self, assets: &str) {
		self.frame_buffer.data.copy_from_slice(unsafe {
			&mut BG_PIXELS.as_mut().unwrap()
		});
		...
		self.surface.damage_buffer(0, 0, self.size.w as i32, self.size.h as i32);
	}
